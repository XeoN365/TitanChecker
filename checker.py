...
class Checker():
    ...
    def addPatient(self,point):
        ...
        while yes_button is None:
            ...
            time.sleep(1)
            yes_button = self.locateCenter(yes_button_image, pyag.screenshot())
            logging.info(f"Looking for yes button. ({tries})")
            if tries > 15:
                return True
            tries += 1
    ...
    def skip(self):
        ...
        while skip_button is None:
            ...
            if tries > 15:
                return True
            tries += 1
        logging.error("Skipping prescription!")
    ...
    def start(self):
        ...
        logging.warn("Starting Checker! Open titan on checking screen!")
        ...
        while self.running:
            try:
                ...
                if clinically_unsuitable is not None:
                    logging.error("Prescription clinically unsuitable!")
                    self.skip()
                
                if seven_day_check is not None:
                    logging.error("Patient has 7 day check!")
                    self.skip()
                    time.sleep(3)
                
                if out_dispense_window is not None:
                    logging.error("Prescription already checked!")
                    self.skip()
                ...
                if do_another_button is not None:
                    logging.info("No checks for this patient! NEXT!")
                ...
                if clinically_unsuitable is None and out_dispense_window is None and seven_day_check is None and post_date is None and patient_no_match is None and drug_doubling is None and drug_therapy is None and do_another_button is None and high_risk_item is None and special_container is None and check_another is None:
                    logging.error("Something went wrong! skipping cycle!")
                    tries += 1
                    if tries > 15:
                        logging.error("Can't find any checks! Looking for skip button!")
                        self.skip()
                        tries = 0
    ...
    def finish(self):
        self.running = False
        logging.info(f"Checks done: {self.checks_done}")
